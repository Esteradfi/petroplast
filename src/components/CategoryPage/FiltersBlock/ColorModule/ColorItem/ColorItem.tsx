...
import { changeColors } from "../../../../../redux/filters-reducer";
...
const ColorItem: React.FC<ColorItemProps> = (props) => {
    ...
    let selectedColors = useAppSelector(state => state.filters.colors.selectedColors);
    let setedColors = useAppSelector(state => state.filters.colors.setedColors);
    let isChecked = false;
    ...
    return (
        <div className={styles.item}>
            <label htmlFor={props.el} className={"custom-radio " + styles.label}>
                <input onChange={onCheckbox} checked={isChecked} type="checkbox" value={props.el} id={props.el} />
                <span className={"custom-radio-span " + styles.checkboxSpan}></span>
